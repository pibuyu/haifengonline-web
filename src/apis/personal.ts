import {
    DeleteChatItemReq,
    GetChatHistoryMsgReq,
    GetChatHistoryMsgRes,
    GetChatListRes,
    PersonalLetterReq,
} from "@/types/personal/chat/chat";
import {
...
import {CheckInRequest, DeleteRecordByIDReq, GetRecordListReq, GetRecordListRes} from "@/types/personal/record/record";
import {changePasswordReq} from "@/types/personal/userInfo/security";
import {
    AttentionReq,
    DetermineNameExistsReq,
    DetermineNameExistsRes,
    SetUserInfoRes,
    UpdateAvatarReq,
    UserInfoRes
} from "@/types/personal/userInfo/userInfo";
...
export const setUserInfoRequist = (params: UserInfoRes) => {
    // console.log("请求发出之前再打印一次",params.gender)
...
//根据收藏夹id获取收藏夹信息（主要是title）
export const getCollectListName = (params: GetCollectListName) => {
    return httpRequest.post<GetCollectListName>('/user/getCollectListName', params);
}
...
export const checkIn = ()=>{
    return httpRequest.post('/user/checkin');
}
export const getIntegral=()=>{
    return httpRequest.post('/user/getIntegral');
}
