...
//私信框中的时间格式化
export const noticeListTimeConversion = (dateStr: string): string => {
    let date = new Date(dateStr).toJSON();
    let newDate: string
    //获取当前时间的日期
    const today = new Date()
    const todayStr = today.toISOString().substring(0, 10)
    //如果私信的日期是今天，只返回'HH:mm'
    if (dateStr.substring(0, 10) == todayStr) {
        newDate = new Date(+new Date(date) + 8 * 3600 * 1000).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');
        newDate = dayjs(newDate).format('HH:mm')
    } else {
        //如果私信日期是昨天，则返回'昨天  HH:mm'
        //取昨天的日期
        const yesterday = new Date(today);
        yesterday.setDate(yesterday.getDate() - 1);
        const yesterdayStr = yesterday.toISOString().substring(0, 10);
        if (dateStr.substring(0, 10) === yesterdayStr) {
            // newDate = '昨天 ' + dateStr.substring(11, 5); // 保留时间部分
            newDate = new Date(+new Date(date) + 8 * 3600 * 1000).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');
            newDate = '昨天 ' + dayjs(newDate).format('HH:mm')
        } else {
            //不是今天也不是昨天，直接返回'YYYY/MM/DD-HH:mm'
            // newDate = dateStr.substring(0, 16).replace('T', ' '); // 只保留日期和时间的部分
            newDate = new Date(+new Date(date) + 8 * 3600 * 1000).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');
            newDate = dayjs(newDate).format('YYYY/MM/DD HH:mm')
        }
    }
    return newDate
}
