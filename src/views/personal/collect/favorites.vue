<template>
  <div class="overall">
        ...
        <el-skeleton style="width: 100%; height: 7.4rem;  margin:  6px 20px 20px 0px;" :loading="!item.id" animated>
          ...
          <template #default>
            <div class="cover">
              ...
              <el-image style="width: 100%; height: 100%;border-radius:50px" :src="item.cover" fit="cover"/>
            </div>
            <div class="info">
              <div class="title">{{ item.title }}</div>
              <div class="creator">创建者 : {{ item.userInfo.username }} 播放数 : 0</div>
              <div class="btn">
                <el-button type="primary" round size="small" @click="viewContent(item.id,item.title)"> 查看内容
                </el-button>
              </div>
            </div>
            <div class="function">
    ...
    <div class="collect-empty" v-show="favoritesList.length == 0 && isLoading == true">
      ...
      <a @click="createCollectDialogShow=true" style="color: #3f9eff;cursor: pointer">立即创建</a>
    </div>
    <div class="dialog">
...
const viewContent = (id: number, title: string) => {
  router.push({
